...
| Qt::QueuedConnection | 队列关联（异步）。当控制返回 receiver所在线程的事件循环后再执行槽,无论槽函数执行与否,发射信号处后面的代码都会立即执行。**槽函数在接收者所在线程执行。** |
...
+ 槽的参数不能多于信号的参数，因为若槽的参数更多，则多余的参数不能接收到信
号传递过来的值，若在槽中使用了这些多余的无值的参数，就会产生错误。
+ 若信号的参数多于槽的参数，则多余的参数将被忽略。
+ 一个信号可以与多个槽关联，多个信号也可以与同一个槽关联，信号也可以关联到
另一个信号上。
